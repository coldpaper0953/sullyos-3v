...
export const OSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 自主后端：事件轮询（主动消息/日记/工具卡片）+ 记忆同步队列后台 flush
  useEffect(() => {
  ...
  useEffect(() => {
      ...
      // 自主后端事件运行时派发的提示：新日记 / 日记便签 / 工具活动卡片。
  ...
  const deleteCharacter = async (id: string, options?: { force?: boolean }): Promise<DeleteCharacterResult> => {
    ...
    // 自主后端：角色删了，后端里那份上下文/记忆也得跟着删，不然 heartbeat 还会
    // 让已删角色继续苏醒。context_not_found 属于「后端本来就没有这个角色」，直接放行。
