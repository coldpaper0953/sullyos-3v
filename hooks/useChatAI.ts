...
export const useChatAI = ({
    ...
    const triggerAI = async (
        ...
        if (!effectiveApi.baseUrl) { addToast('请先在设置 → API 配置填好 API URL，角色才能回复', 'error'); onInstantPosted?.(); return; }
        ...
        try {
            // 故障转移轮换时这两个随候选 API 一起换（attempt 回调里重新赋值）。
            let baseUrl = effectiveApi.baseUrl.replace(/\/+$/, '');
            const headers: { 'Content-Type': string; Authorization: string } = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${effectiveApi.apiKey || 'sk-none'}` };
            ...
            let data: any;
            try {
                // 自动故障转移：429/网络错误按用户配置重试 N 次，仍失败就轮换到
                // 下一条 API 预设（设置 → API 故障转移 可调重试次数/开关）。
                // 整个「主请求 + Claude/MCP 兼容重试」作为一个 attempt 单元——
                // 兼容重试只对请求形状做文章，换候选 API 时三件套一起换。
                data = await runWithApiFailover({
                    primary: { baseUrl: effectiveApi.baseUrl, apiKey: effectiveApi.apiKey, model: effectiveApi.model },
                    attempt: (failoverCandidate) => {
                        baseUrl = failoverCandidate.baseUrl.replace(/\/+$/, '');
                        headers.Authorization = `Bearer ${failoverCandidate.apiKey || 'sk-none'}`;
                        baseReqBody.model = failoverCandidate.model;
                        return (async () => {
                            let inner: any;
                            try {
                                inner = await safeFetchJson(`${baseUrl}/chat/completions`, {
                                    method: 'POST', headers,
                    ...
                    onSwitch: (candidate, reason) => {
                        console.warn(`🔁 [API 故障转移] 切换到「${candidate.name}」(${candidate.baseUrl})：${reason}`);
                        addToast(`API 已自动切换到「${candidate.name}」`, 'info');
                    },
                });
            } catch (e) {
                // runWithApiFailover 的最后一候选失败会原样抛出；到这里说明全部候选耗尽。
                // 历史上这里还承载流式预览的收尾（streamHooks 已在 safeFetchJson 内处理重置），
                // 直接向上抛给统一的 catch 处理链。
                throw e;
            }
            ...
            if (backendConfig.enabled && backendConfig.baseUrl.trim() && backendConfig.token.trim()) {
                void Promise.all([
                    ...
                    console.warn('[Backend Sync] 聊天增量同步失败，将由后续同步补齐：', error);
                });
            }
