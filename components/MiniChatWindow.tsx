...
const loadPos = (): { x: number; y: number } => {
    ...
    return { x: 8, y: 4 }; // 默认左上角
};

/**
 * 左上角未读红点 + 通讯录弹窗 + 可拉起的小聊天窗。
 * - 有未读私聊消息时左上角显示红点（数字），红点可拖动（位置记忆在 localStorage）；
...
const MiniChatWindow: React.FC = () => {
    ...
    // 拖动红点：pointerdown 记录起点，pointermove 更新，pointerup 收尾（区分点击 vs 拖动）
    useEffect(() => {
        if (!dragging) return;
        const move = (ev: PointerEvent) => {
            movedRef.current = true;
            setPos({
                x: Math.max(0, Math.min(window.innerWidth - 24, ev.clientX - dragOffsetRef.current.x)),
                y: Math.max(0, Math.min(window.innerHeight - 24, ev.clientY - dragOffsetRef.current.y)),
            });
        };
        ...
        <>
            ...
            {totalUnread > 0 && !open && (
                <button
                    ...
                    onPointerUp={() => {
                        setDragging(false);
                        if (!movedRef.current) { setOpen(true); setView('contacts'); }
                    }}
                    title={`${totalUnread} 条未读 · 点击打开通讯录（可拖动位置）`}
                    style={{ left: pos.x, top: pos.y, touchAction: 'none' }}
                    className={`fixed z-[95] min-w-[18px] h-[18px] px-1 rounded-full bg-rose-500 text-white text-[9px] font-black shadow-md border border-white/60 flex items-center justify-center ${dragging ? '' : 'animate-pulse'} ${movedRef.current ? 'cursor-grabbing' : 'cursor-grab'}`}
                >
                    {totalUnread > 99 ? '99+' : totalUnread}
                </button>
            )}

            {/* 通讯录列表弹窗：列出谁发来了未读，可忽略 / 私聊 / 跳共同群 */}
            {open && view === 'contacts' && (
                ...
                <div className="fixed z-[96] right-2 bottom-16 w-80 h-[26rem] max-h-[70vh] rounded-2xl overflow-hidden shadow-2xl border border-slate-200 bg-white flex flex-col animate-fade-in">
                    <div className="shrink-0 px-3 py-2 bg-slate-50 border-b border-slate-200/70 flex items-center gap-2">
                        <button onClick={() => setView('contacts')} title="返回通讯录列表"
                            className="w-6 h-6 rounded-full bg-white border border-slate-200 text-slate-400 flex items-center justify-center active:scale-90">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3"><path d="M14.5 5.5L8 12l6.5 6.5" /></svg>
                        </button>
                        <span className="text-xs font-bold text-slate-600 flex-1 truncate">{charOf(targetChar)?.name || '私聊'}（小窗私聊）</span>
