...
const ChatInputArea: React.FC<ChatInputAreaProps> = ({
    ...
    // 拍照：getUserMedia 实时取景弹层（桌面/手机一致体验），不再用 input capture
                    ...
                    {showPanel === 'actions' && !actionsContent && (
                        ...
                        >
                          ...
                          <div className={`p-6 grid grid-cols-4 gap-8 ${actionsPage === 2 ? '' : 'hidden'}`}>
                            {/* 拍照：getUserMedia 实时取景，快门后走与相册相同的发送管道 */}
                            <button onClick={() => { setCameraOpen(true); trackEvent('打开聊天拍照'); }} className={`flex flex-col items-center gap-2 active:scale-95 transition-transform ${acnh ? 'text-[#725d42]' : isDiscordStyle ? 'text-slate-200' : 'text-slate-600'}`}>
                                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm border ${acnh ? 'bg-white/70 border-[#e6dab4] text-[#4cb89e]' : isDiscordStyle ? 'bg-slate-800 text-emerald-300 border-emerald-400/20' : 'bg-emerald-50 text-emerald-500 border-emerald-100'}`}>
                            ...
                            {cameraOpen && createPortal(
                                <CameraCapture
                                    ...
                                    onCapture={(file) => { setCameraOpen(false); onImageSelect(file); }}
                                />,
                                document.body,
                            )}
