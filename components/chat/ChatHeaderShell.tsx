...
interface ChatHeaderShellProps {
    ...
    /** 聊天内快捷切换 API：不传则不渲染（群聊等复用方保持原样） */
    apiSwitcher?: React.ReactNode;
        ...
        <div className={`sully-chat-header ${headerDensityClass} flex items-center relative ${headerToneClass}`} style={headerSafeStyle}>
            ...
            {selectionMode ? (
                ...
                <div className="relative w-full min-h-[56px] flex items-end justify-center">
                    ...
                    {apiSwitcher && (
                        <div className="absolute right-[3.25rem] bottom-2">
                            {apiSwitcher}
                        </div>
                    )}
                ...
                <div className="flex items-center gap-3 w-full">
                    ...
                    {apiSwitcher}
                    <button onClick={onTriggerAI} className={`sully-chat-trigger p-2 ${extraAction || apiSwitcher ? '' : 'ml-auto'} ${actionButtonClass}`} title={triggerIcon === 'stop' ? '停止生成' : '触发 AI'}>
                        {triggerIconNode}
                    </button>
                </div>
            )}
