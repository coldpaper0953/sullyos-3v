...
const SocialApp: React.FC = () => {
    ...
    // 评论延迟生成的计时器（打开详情页后停留几秒再请求）
    const commentDelayTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    // 自动刷新间隔循环里调用的是「最新版」handleRefresh（普通闭包会捕获旧 state）
    const handleRefreshRef = useRef<(() => Promise<void>) | null>(null);
    // 评论延迟（毫秒）：点开帖子后停留多久才发 AI 评论请求。spark_comment_delay_ms 可调，默认 4000（3-5 秒区间中值）。
    // 注意 Number(null) === 0：必须先排除 null/空串，否则首开用户会看到 0.0 秒而不是默认值。
    const [commentDelayMs, setCommentDelayMs] = useState<number>(() => {
        const raw = localStorage.getItem('spark_comment_delay_ms');
        const parsed = raw === null ? Number.NaN : Number(raw);
        return Number.isFinite(parsed) && parsed >= 0 ? parsed : 4000;
    });
    // 帖子流自动刷新（分钟）：0 = 关。不依赖用户发消息，进 Spark 每隔 N 分钟自动拉一批新帖。
    const [autoRefreshMinutes, setAutoRefreshMinutes] = useState<number>(() => {
        const raw = localStorage.getItem('spark_auto_refresh_minutes');
        const parsed = raw === null ? Number.NaN : Number(raw);
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (commentDelayTimerRef.current) {
                clearTimeout(commentDelayTimerRef.current);
    ...
    // 帖子流自动更新：与用户是否发消息解耦，进 Spark 后每 N 分钟自动拉一批新帖
    // 垫在推荐流底部（不整流刷新，避免正在看的帖子跳位置）。autoRefreshMinutes = 0 即关闭。
    ...
    useEffect(() => {
        ...
        // handleRefresh 内部已有 refreshRequestRef 防重入；这里直接复用整套生成逻辑。
        autoRefreshRef.current = setInterval(() => {
            // 详情页/创作面板打开时打断体验，跳过本轮；离开应用组件的间隔循环由 unmount 清理兜底。
            if (!mountedRef.current || selectedPost || isCreateOpen) return;
    ...
    }, [autoRefreshMinutes, selectedPost, isCreateOpen]);
...
[
  ...
  ...
    ...
    // 把最新版 handleRefresh 交给自动刷新 interval（每次渲染后对齐一次）
    useEffect(() => {
...
[
  { "author": "网名 (Handle)", "content": "回复内容..." }
    ...
    const handleOpenPost = (post: SocialPost) => {
        ...
        // 停留 3-5 秒再触发生成：打开详情页先让用户安静看帖子本身，
        // 延迟时长可调（spark_comment_delay_ms，毫秒，默认 4000）。
        if (commentDelayTimerRef.current) clearTimeout(commentDelayTimerRef.current);
        commentDelayTimerRef.current = setTimeout(() => {
            commentDelayTimerRef.current = null;
            // 用户可能早已关掉详情页——关掉就不生成，别浪费一次请求。
            setSelectedPost(current => {
                if (current?.id === livePost.id) {
                ...
                <div className="space-y-6">
                    <div className="max-h-[50vh] overflow-y-auto no-scrollbar space-y-6 px-1">
                        ...
                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm space-y-3">
                            <label className="text-[9px] text-slate-400 uppercase font-bold">评论延迟生成（毫秒）</label>
                            <div className="flex items-center gap-2">
                                <input
                                    type="range"
                                    min={0}
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setCommentDelayMs(v);
                                        localStorage.setItem('spark_comment_delay_ms', String(v));
                                    }}
                                    className="flex-1 accent-[#ff2442]"
                                />
                                <span className="text-[10px] font-bold text-slate-500 w-14 text-right">{(commentDelayMs / 1000).toFixed(1)} 秒</span>
                            </div>
                            <p className="text-[10px] text-slate-400">点进帖子后停留该时长再生成 AI 评论；0 = 立即生成。</p>

                            <label className="text-[9px] text-slate-400 uppercase font-bold pt-1 block">推荐流自动更新（分钟）</label>
                            <div className="flex items-center gap-2">
                                <input
                                    type="range"
                                    min={0}
                                    ...
                                    onChange={(e) => {
                                        ...
                                        localStorage.setItem('spark_auto_refresh_minutes', String(v));
                                    }}
                                    className="flex-1 accent-[#ff2442]"
                                />
                                <span className="text-[10px] font-bold text-slate-500 w-14 text-right">{autoRefreshMinutes === 0 ? '关闭' : `${autoRefreshMinutes} 分`}</span>
                            </div>
                            <p className="text-[10px] text-slate-400">不依赖发消息：开着 Spark 时每 N 分钟自动刷一批新帖；0 = 只手动刷新。</p>
                        </div>
                        {/* 分组筛选（没建分组时不渲染） */}
