...
const Settings: React.FC = () => {
  ...
  // API 故障转移：429/网络错误按次数重试，耗尽后轮换到下一条预设（utils/apiFailover.ts）
  const [failoverEnabled, setFailoverEnabled] = useState(() => loadApiFailoverConfig().enabled);
  const [failoverRetries, setFailoverRetries] = useState(() => loadApiFailoverConfig().retriesPerApi);
    ...
    <div className="h-full w-full bg-[#f3f4f8] flex flex-col font-light relative isolate">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 no-scrollbar pb-20">
        ...
        >
            ...
            {/* API 自动故障转移：429/网络错误按次数重试，耗尽后自动轮换到下一条预设 */}
            <div className="mb-4 rounded-2xl bg-slate-50/70 border border-slate-100 p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">API 故障转移</span>
                        <p className="text-[9px] text-slate-300 mt-0.5">429/超时重试后自动切换到「我的预设」里的下一条</p>
                    </div>
                    <button
                        type="button"
                        onClick={() => {
                            ...
                            saveApiFailoverConfig({ enabled: next, retriesPerApi: failoverRetries });
                        }}
                        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors shrink-0 ${failoverEnabled ? 'bg-primary' : 'bg-slate-200'}`}
                    >
                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${failoverEnabled ? 'translate-x-4' : 'translate-x-0.5'}`} />
                    </button>
                </div>
                {failoverEnabled && (
                    <div className="mt-3 pt-3 border-t border-slate-100">
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] text-slate-400">重试次数（每条 API）</span>
                            <span className="text-[10px] font-mono text-slate-400">{failoverRetries} 次</span>
                        </div>
                        <input
                            type="range"
                            min="0"
                            max="5"
                            step="1"
                            value={failoverRetries}
                            onChange={(e) => {
                                const v = parseInt(e.target.value, 10) || 0;
                                setFailoverRetries(v);
                                saveApiFailoverConfig({ enabled: true, retriesPerApi: v });
                            }}
                            className="w-full accent-primary mt-1"
                        />
                        <p className="text-[9px] text-slate-300 mt-0.5">0 = 不重试直接切换；401/403/404 一律直接切换不重试</p>
                    </div>
                )}
            </div>
