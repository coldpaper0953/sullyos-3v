...
import ApiQuickSwitcher from '../components/chat/ApiQuickSwitcher';
...
const Chat: React.FC = () => {
    ...
    const handleReroll = async () => {
        ...
        // 删除的 tombstone 已在 DB.deleteMessages 的事务里入队；这里立即尝试 flush，
        // 后端不可用时留在队列里由下次同步自动补偿。
        const rerollBackendConfig = loadBackendChatConfig();
        if (rerollBackendConfig.enabled) {
            void flushBackendMemorySyncQueue({
                config: rerollBackendConfig,
                character: char,
                user: userProfile,
                priorityDeletedMessageIds: toDeleteIds,
                priorityDeletedEventIds: [],
            }).catch(error => console.warn('[Chat] backend deletion deferred:', error));
        }
    ...
    // 日程表也随自主后端的上下文走（后端 heartbeat 按它决定角色此刻在干嘛），改完同步一份。
        ...
        >
             ...
             <ChatHeader
                ...
                apiSwitcher={
                    <ApiQuickSwitcher
                        presets={apiPresets}
                        activePresetId={chatActivePresetId}
                        addToast={addToast}
                        onSelect={(preset) => {
                            updateApiConfig(configFromPreset(preset));
                            addToast(`已切换到「${preset.name}」，立即生效`, 'success');
                            trackEvent('聊天内快捷切换 API', { 预设: preset.name });
                        }}
                    />
                }
                onDeleteBuff={(buffId) => {
